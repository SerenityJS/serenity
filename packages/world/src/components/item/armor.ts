import { EquipmentSlot, LevelSoundEvent, LevelSoundEventPacket, Vector3f } from "@serenityjs/protocol";
...
class ItemArmorComponent<T extends keyof Items> extends ItemComponent<T> {
  ...
  public onUse(player: Player, cause: ItemUseCause): void {
    ...
    if (playerArmorComponent.getEquipment(itemEquipmentSlot)) {
      ...
      playerInventoryComponent.container.setItem(playerInventoryComponent.selectedSlot, equipedItem!);
      return;
    }
    // Clear the selected slot, and set the used item into the players armor inventory
    playerInventoryComponent.container.clearSlot(playerInventoryComponent.selectedSlot);
    playerArmorComponent.setEquipment(itemEquipmentSlot, player.usingItem);
  }

  // TODO: Clean up function
  protected resolveEquipmentSlot(itemStack: ItemStack): EquipmentSlot | undefined {
    if (!itemStack) return;
    ...
    return Object.entries(suffixToSlotMap).find(([suffix]) => itemStack.type.identifier.endsWith(suffix))?.[1];
  }
}
