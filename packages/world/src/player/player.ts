...
import type { PlayerAbilityComponents, PlayerComponents } from "../types/components";
...
class Player extends Entity {
  ...
  public sync(): void {
    ...
    const filtered = available.commands.filter((command) => command.permissionLevel <= this.permission);
  ...
  public spawn(player?: Player): void {
    ...
    packet.heldItem = heldItem === null ? new NetworkItemStackDescriptor(0) : ItemStack.toNetworkStack(heldItem);
    packet.gamemode = 0;
   ...
   */
  public hasComponent<T extends keyof PlayerComponents>(identifier: T): boolean {
   ...
   */
  public getComponent<T extends keyof PlayerComponents>(identifier: T): PlayerComponents[T] {
   ...
   */
  public removeComponent<T extends keyof PlayerComponents>(identifier: T): void {
   ...
   */
  public getAbility<T extends keyof PlayerAbilityComponents>(ability: T): PlayerAbilityComponents[T] {
  ...
  public getAbilities(): Array<PlayerAbilityComponent> {
    return [...this.components.values()].filter(
      (component): component is PlayerAbilityComponent => component instanceof PlayerAbilityComponent
    );
  }
