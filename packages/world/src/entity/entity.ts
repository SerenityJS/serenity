...
class Entity {
  ...
  public constructor(identifier: EntityIdentifier, dimension: Dimension, uniqueId?: bigint) {
    // Readonly properties
    this.type = EntityType.get(identifier) as EntityType;
    this.runtime = Entity.runtime++;
    this.unique = uniqueId ? (uniqueId < 0n ? -uniqueId >> 32n : uniqueId >> 32n) : Entity.runtime;
    ...
    for (const component of EntityComponent.registry.get(identifier) ?? []) new component(this, component.identifier);
  }
  ...
  public executeCommand(command: string): CommandResult | undefined {
    ...
    const state = new CommandExecutionState(this.dimension.world.commands, command, this);
    ...
    try {
      ...
      // Check if the entity is a player
      if (this.isPlayer()) {
        this.dimension.world.logger.error(`Failed to execute command '${command}' for player '${this.username}':`, reason);
      } else {
   ...
   */
  public hasComponent<T extends keyof EntityComponents>(identifier: T): boolean {
   ...
   */
  public getComponent<T extends keyof EntityComponents>(identifier: T): EntityComponents[T] {
   ...
   * @param identifier The identifier of the component.
   */
  public removeComponent<T extends keyof EntityComponents>(identifier: T): void {
   ...
   * @param sync Whether to synchronize the metadata.
   */
  public setData(key: MetadataKey, value: bigint | boolean | number | string, sync = true): void {
   ...
   * @param sync Whether to synchronize the metadata.
   */
  public createData(key: MetadataKey, value: bigint | boolean | number | string, type: MetadataType, sync = true): void {
  ...
  public createFlag(flag: MetadataFlags, value: boolean, sync = true): void {
    // Create a new MetadataDictionary
    const data = new MetadataDictionary(MetadataKey.Flags, MetadataType.Long, value, flag);
  ...
  public hasAttribute(name: AttributeName): boolean {
    return [...this.attributes.values()].some((attribute) => attribute.name === name);
  }
  ...
  public getAttribute(name: AttributeName): Attribute | undefined {
    return [...this.attributes.values()].find((attribute) => attribute.name === name);
  }
  ...
  public createAttribute(
    ...
    const attribute = new Attribute(value, defaultValue, maxValue, minValue, [], name);
  ...
  public addEffect(effect: Effect): void {
    const effects = this.hasComponent("minecraft:effects") ? this.getComponent("minecraft:effects") : new EntityEffectsComponent(this);
    effects.add(effect);
  }
  ...
  public getEffects(): EffectType[] {
    const effects = this.hasComponent("minecraft:effects") ? this.getComponent("minecraft:effects") : new EntityEffectsComponent(this);
    return Array.from(effects.effects.keys());
  }
