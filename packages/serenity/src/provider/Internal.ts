...
import { WorldProvider } from './Provider.js';
...
class InternalProvider extends WorldProvider {
	...
	public override readChunk(cx: number, cz: number, dimension: Dimension): Chunk {
		...
		if (!chunks.has(Chunk.getHash(cx, cz))) {
			chunks.set(Chunk.getHash(cx, cz), dimension.generator.apply(new Chunk(cx, cz)));
		}
