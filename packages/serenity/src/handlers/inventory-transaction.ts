...
class InventoryTransaction extends SerenityHandler {
  ...
  public static handle(packet: InventoryTransactionPacket, session: NetworkSession): void {
  ...
  public static handleNormalTransaction(actions: Array<InventoryAction>, player: Player): void {
    // TODO: CLEANUP
    ...
    // Spawn the entity
    const entity = player.dimension.spawnItem(item, new Vector3f(x, y - 0.25, z));

    // Set the velocity of the entity
    entity.setMotion(velocity);
  }

  public static handleItemUseTransaction(transaction: ItemUseInventoryTransaction, player: Player): void {
    ...
    switch (transaction.type) {
      case ItemUseInventoryTransactionType.Place: {
        ...
        if (usingItem.type.network !== transaction.item.network) throw new Error("Item network ids do not match.");
        ...
        const canBuild = player.getComponent("minecraft:ability.build").currentValue;
        ...
        const blockPermutation = blockType.permutations[usingItem.metadata] ?? blockType.getPermutation();
      ...
      case ItemUseInventoryTransactionType.Use: {
        ...
        const usingItem = playerInventory.container.getItem(playerInventory.selectedSlot);
      ...
      case ItemUseInventoryTransactionType.Destroy: {
        this.serenity.logger.debug("ItemUseInventoryTransactionType.Destroy is not implemented yet.", transaction);
      }
    }
  }

  public static handleItemUseOnEntityTransaction(transaction: ItemUseOnEntityInventoryTransaction, player: Player): void {
    // Get the entity from the dimension
    const entity = player.dimension.getEntityByRuntime(transaction.actorRuntimeId);
