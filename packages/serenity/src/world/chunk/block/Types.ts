...
export class BlockTypes {
	...
	public get(typeId: string): BlockType | null {
		return this.types.get(typeId.includes(':') ? typeId : 'minecraft:' + typeId) ?? null;
	}
	public getAll(): IterableIterator<BlockType> {
	...
	public constructor(rawBlockTypes: Buffer) {
		...
		for (const key of Object.keys(blockTypes)) {
			...
			for (const permutation of blockType.permutations.values())
