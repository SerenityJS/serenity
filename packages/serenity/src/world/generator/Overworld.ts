...
class Overworld extends TerrainGenerator {
	...
	public constructor() {
		...
		this.foilageNoise = createNoise2D(() => this.seed * 0.133_7);
	...
	public apply(chunk: Chunk): Chunk {
		...
		const tulip_pink = BlockPermutation.resolve('minecraft:red_flower', { flower_type: 'tulip_pink' });
		...
		const cornflower = BlockPermutation.resolve('minecraft:red_flower', { flower_type: 'cornflower' });
		...
		for (let x = 0; x < 16; x++) {
			for (let z = 0; z < 16; z++) {
				const height = Math.floor(60 + 20 * this.worldNoise((chunk.x * 16 + x) * 0.005, (chunk.z * 16 + z) * 0.005));
				...
				if (height <= 62)
					for (let y = 0; y < 62; y++) {
						if (chunk.getPermutation(x, y, z).type.identifier === 'minecraft:air') {
							chunk.setPermutation(x, y, z, water);
						}
					}
				else {
					const foliageMap = Math.floor(
						55 + 20 * this.foilageNoise((chunk.x * 16 + x) * 0.1, (chunk.z * 16 + z) * 0.1),
					);

					// TODO: Do this properly
					if (foliageMap > 60 && foliageMap !== 67 && foliageMap !== 72 && foliageMap !== 78) {
						if (foliageMap < 65) chunk.setPermutation(x, height + 1, z, tulip_pink);
						else if (foliageMap < 70) chunk.setPermutation(x, height + 1, z, cornflower);
						else if (foliageMap < 75) chunk.setPermutation(x, height + 1, z, dandelion);
						else chunk.setPermutation(x, height + 1, z, poppy);
					}

					// We dont want trees next to each other
					const treeMap = Math.floor(55 + 20 * this.treeNoise((chunk.x * 16 + x) * 0.72, (chunk.z * 16 + z) * 0.72));
