...
import { Item, type Player } from '../../index.js';
...
class PlayerActionHandler extends NetworkHandler {
	...
	private static handleAbortBreak(packet: PlayerAction, player: Player): void {
		// Get the block position from the packet.
		const { x, y, z } =
			packet.blockPosition === player.mining ? packet.blockPosition : player.mining ?? { x: 0, y: 0, z: 0 };
	...
	private static handlePredictBreak(packet: PlayerAction, player: Player): void {
	...
	private static handleContinueBreak(packet: PlayerAction, player: Player): void {
		...
		if (player.mining) {
			...
			event.event = LevelEventId.BlockStopBreak;
			event.position = new Vector3f(player.mining.x, player.mining.y, player.mining.z);
			event.data = 0;
		...
		// Get the block from the dimension.
		const block = player.dimension.getBlock(packet.blockPosition.x, packet.blockPosition.y, packet.blockPosition.z);
		...
		const event = new LevelEvent();
		event.event = LevelEventId.BlockStartBreak;
		event.position = new Vector3f(packet.blockPosition.x, packet.blockPosition.y, packet.blockPosition.z);
		event.data = 65_535 / breakTime;
	...
	private static handleStopItemUseOn(packet: PlayerAction, player: Player): void {
