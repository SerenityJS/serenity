...
class JsonEnum extends ValidEnum {
	/**
	 * The type of the enum.
	 */
	public static readonly name = "json";
		...
		if (typeof text === "string") {
			...
			if (text.startsWith('"')) {
				...
				if (final.at(-1)?.endsWith('"'))
					return new JsonEnum(final.join(" ").slice(1).slice(0, -1));
				...
				if (this.options.includes(text)) return new JsonEnum(text);
				else {
