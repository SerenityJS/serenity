...
class Zip {
  ...
  private writeLocalFileHeader(header: LocalFileHeader): void {
    ...
    this.stream.writeUint32(Zip.LOCAL_FILE_HEADER_SIG, Endianness.Little);
  ...
  public compressPack(): { data: Buffer; originalSize: bigint } {
    ...
    const files = this.readDirectory(this.dirPath, "", true);
  ...
  public getPackSize(): bigint {
    const files = this.readDirectory(this.dirPath, "", false);
