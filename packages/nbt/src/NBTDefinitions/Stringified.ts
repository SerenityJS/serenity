...
import type { NBTSerializable, NBTValue
} from "../NBT";
import { 
    NBTTag, Byte, Int16, Int32, Int64, Float, Double, DefinitionReader, DefinitionWriter,
    NBT_SERIALIZER, NBT_TYPE
} from "../NBT";
...
class StringifiedNBTDefinitionWriter extends DefinitionWriter{
    ...
    public [NBTTag.Byte](value: number): void { this.text += value + "b"; }
    public [NBTTag.Int16](value: number): void { this.text += value + "s"; }
    public [NBTTag.Int32](value: number): void { this.text += value + "i";}
    public [NBTTag.Float](value: number): void { this.text += value + "f"; }
    public [NBTTag.Double](value: number): void { this.text += value + "d"; }
    public [NBTTag.Int64](value: bigint): void { this.text += value + "l"; }
    ...
    public [NBTTag.Int32Array](value: any): any{ throw new Error("No implementation");};
    public [NBTTag.Int64Array](value: any): any{ throw new Error("No implementation");};
...
class StringifiedNBTDefinitionReader extends DefinitionReader{
    ...
    public [NBTTag.Int32Array](): any{ throw new Error("Method not implemented.");};
    public [NBTTag.Int64Array](): any{ throw new Error("Method not implemented.");};
...
class Source extends String{
    ...
    public [Symbol.iterator](){return {next:()=>({done: this.offset >= this.length, value: this[this.offset++]})} as IterableIterator<string>; }
}
...
namespace SNBT{
    ...
    function readCompoud(source: Source){
        ...
        for (let char of source) {
            if(char === '}') return obj;
            else if(!firsObj && char !== ",") { throw new SyntaxError("Unexpected: " + char); }
    ...
    function readArray(source: Source){
        ...
        for (let char of source) {
            ...
            else if(initialKind !== value[NBT_TYPE]) throw new TypeError("Array could have just one kind of elements, but multiple, expected: " + NBTTag[initialKind] + " but got: "+ NBTTag[value[NBT_TYPE]]);
            obj.push(value);
            readWhiteSpace(source);
            firsObj = false;
        }
...
class StringifiedNBT extends NBT{
    public static Stringify(tag: NBTSerializable, spacing?: string){ return new StringifiedNBTDefinitionWriter().Stringify(tag,spacing);}
    public static Parse(text: string){ return new StringifiedNBTDefinitionReader().Parse(text);}
}
