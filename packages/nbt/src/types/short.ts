...
class ShortTag extends Number implements BaseTag {
  public readonly type = TagType.Short;
  ...
  public static read(
    ...
    if (options?.type) {
      ...
      if (type !== TagType.Short) {
        ...
        throw new Error(
          `Expected tag type to be ${TagType[TagType.Short]}, received ${TagType[type] ?? type}.`
        );
      }
    }
    ...
    const value = stream.readInt16(Endianness.Little);

    // Create and return a new ShortTag instance.
  ...
  public static write(
    ...
    stream.writeInt16(value.valueOf(), Endianness.Little);
  }
}
