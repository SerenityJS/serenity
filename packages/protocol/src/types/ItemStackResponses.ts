...
import type { ItemStackStatus, ContainerSlotType } from '../enums/index.js';
...
class ItemStackResponses extends DataType {
	...
	public id: number;
	public containers?: ItemStackContainer[];

	public constructor(status: number, id: number, containers?: ItemStackContainer[]) {
		super();
		this.status = status;
		this.id = id;
	...
	public static override read(stream: BinaryStream): ItemStackResponses[] {
		// Prepare an array to store the stacks.
		const stacks: ItemStackResponses[] = [];
		...
		for (let i = 0; i < amount; i++) {
			...
			if (status > 0) {
				...
				const containers: ItemStackContainer[] = [];
				...
				for (let i = 0; i < slotAmount; i++) {
					...
					for (let i = 0; i < containerAmount; i++) {
						...
						// Push the container.
						slots.push({ amount, nametag, durabilityCorrection, hotbarSlot, runtimeId, slot });
					}
				}
	...
	public static override write(stream: BinaryStream, value: ItemStackResponses[]): void {
