...
export class PlayerAuthInputPacket extends DataPacket { 
  ...
  public itemStackRequest!: ItemStackRequest[] | undefined;
  /** If PlayMode == Reality */
  ...
  public predictedVehicle!: ClientPredictedVehicle | undefined;
    ...
    if(this.playMode == PlayMode.Reality) {
      if(!this.gazeDirection) throw new Error("GazeDirection is not defined but Reality flag is set.");
    ...
    if(this.inputData.getFlag(InputDataFlags.ItemInteract)) {
      if(!this.transaction) throw new Error("Transaction is not defined but ItemInteract flag is set.")
      InputTransaction.write(this, this.transaction);
    }    
        
    if(this.inputData.getFlag(InputDataFlags.ItemStackRequest)) {
      if(!this.itemStackRequest) throw new Error("ItemStackRequest is not defined but ItemStackRequest flag is set.");
    ...
    if(this.inputData.getFlag(InputDataFlags.ClientPredictedVehicle)) {
      if(!this.predictedVehicle) throw new Error("PredictedVehicle is not defined but ClientPredictedVehicle flag is set.");
