...
import { ENTITY_COMPONENTS, Entity } from "../../entity";
import { Player } from "../../player";
import { Block, Chunk } from "../chunk";

import type { DataPacket, MetadataFlags } from "@serenityjs/protocol";
import type { EntityComponent } from "../../entity";
...
class Dimension {
	...
	public spawnEntity(identifier: string, position: Vector3f): Entity {
		...
		const components =
			ENTITY_COMPONENTS[identifier as keyof typeof ENTITY_COMPONENTS] ??
			ENTITY_COMPONENTS["minecraft:generic"];
	...
	public getChunksToRender(x: number, z: number): Array<Chunk> {
		// Prepare an array to store the chunks
		const chunks = [];
