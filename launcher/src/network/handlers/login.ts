...
import type { Packet, LoginTokens } from "@serenityjs/protocol";
// import type { PlayerComponent } from "../../player";
...
class LoginHandler extends NetworkHandler {
	...
	public static override handle(packet: Login, session: NetworkSession): void {
		...
		const check = [...this.serenity.worlds.values()].find((world) =>
			world.getPlayers().find((x) => x.xuid === xuid)
		);
		if (check) {
			// If there is, disconnect the player trying to connect.
			return session.disconnect(
				"You are already logged in another location.",
		...
		session.sendImmediate(login, packs);
	}
