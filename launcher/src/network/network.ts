...
class Network extends Emitter<NetworkEvents> {
	...
	public constructor(serenity: Serenity) {
		...
		this.packetsPerFrame =
			serenity.properties.values.network["packets-per-frame"];
		this.compressThreshold =
			serenity.properties.values.network["compression-threshold"];
		this.compressMethod =
			serenity.properties.values.network[
				"compression-method"
			].toLocaleLowerCase() === "zlib"
				? CompressionMethod.Zlib
				: serenity.properties.values.network[
							"compression-method"
					  ].toLocaleLowerCase() === "snappy"
	 ...
	 */
	public incoming(session: NetworkSession, ...payloads: Array<Buffer>): void {
		try {
			...
			for (const buffer of payloads) {
				...
				if (buffer[0] !== GAME_BYTE[0])
					return this.logger.error("Invalid packet header", buffer[0]);
				...
				if (frames.length > this.packetsPerFrame) {
					...
					this.logger.warn(
						`Too many packets sent from "${session.identifier.address}:${session.identifier.port}"!`
					);

					// Disconnect the session if too many packets were sent at once.
					return session.disconnect(
						"Received too many packets sent at once.",
			...
			// If an error occurs, we will emit the error event.
			this.logger.error("Failed to handle incoming packets!", reason, payloads);
		}
	}
